...
export class MentalMarsScraper implements SourceScraper {
  ...
  private extractCodesFromHtml(
    ...
    for (
      let i = 0;
      i < tables.length && i < config.platformOrderedTables.length;
      i++
    ) {
      const platform = config.platformOrderedTables[i];
      if (platform === "discard") continue;

      const tableCodes = this.extractCodesFromTable(
        tables[i],
        config,
        platform
      );
      console.log(
        `[mentalmars] Table ${i} (${platform}): found ${tableCodes.length} codes`
      );
  ...
  private extractCodesFromTable(
    ...
    const headerMatch =
      /<thead[^>]*>[\s\S]*?<tr[^>]*>([\s\S]*?)<\/tr>[\s\S]*?<\/thead>/i.exec(
        tableContent
      );
    if (headerMatch) {
      ...
      console.log(
        `[mentalmars] Mapped columns: Reward=${rewardIdx}, Code=${codeIdx}, Expires=${expiresIdx}`
      );
    } else {
    ...
    while ((rowMatch = rowRegex.exec(rowsContent)) !== null) {
      ...
      const codeMatch = codeRegex.exec(cols[codeIdx]);
