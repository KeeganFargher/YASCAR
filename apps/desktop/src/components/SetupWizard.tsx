...
import { Monitor, Gamepad, Calendar, Zap, Check, ChevronRight, ChevronLeft, Shield } from 'lucide-react';
...
export function SetupWizard({ onComplete }: SetupWizardProps) {
    ...
    const [platforms, setPlatforms] = useState<Platform[]>(DEFAULT_CONFIG.platforms);
    const [autoRedeem, setAutoRedeem] = useState(DEFAULT_CONFIG.autoRedeem);
    const [checkInterval, setCheckInterval] = useState(DEFAULT_CONFIG.checkIntervalMinutes);
    ...
    const steps = [
        {
            title: "TRACKED GAMES",
            icon: Gamepad,
            description: "Select which Borderlands games you want to track codes for."
        },
        {
                ...
                <div className="w-full max-w-4xl">
                    ...
                    <div className="mb-12 text-center">
                        ...
                        <div className="flex items-center justify-center gap-2">
                            {steps.map((s, i) => (
                                <div key={i} className={`flex items-center ${i < steps.length - 1 ? 'w-24' : ''}`}>
                                    <div className={`
                                    ...
                                    {i < steps.length - 1 && (
                                        <div className={`flex-1 h-0.5 mx-2 transition-colors duration-300 ${i < step ? 'bg-bl-yellow' : 'bg-bl-gray-dark'}`} />
                                    )}
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="card-bl shadow-2xl backdrop-blur-md flex flex-col">
                        <div className="p-6 border-b border-bl-gray-dark/50 flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-display text-white tracking-wider">{steps[step].title}</h2>
                                <p className="text-sm text-bl-gray-light">{steps[step].description}</p>
                            </div>
                        </div>
                                    ...
                                    {Object.values(GameTitle).map((game) => (
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <span className={`font-bold text-sm tracking-wide ${games.includes(game) ? 'text-white' : 'text-bl-gray-light'}`}>
                                                    {game}
                                                </span>
                                                <div className={`
                                                ...
                                                `}>
                                                    {games.includes(game) && <Check className="w-3 h-3" />}
                                                </div>
                                            </div>
                                            {/* Corner Accents */}
                            ...
                            {step === 1 && (
                                <div className="space-y-4">
                                    <div className="p-3 bg-blue-900/20 border border-blue-500/30 text-blue-100 rounded mb-4 flex gap-3">
                                        ...
                                        <p className="text-xs">
                                            Tip: Usually "Universal" is all you need. Only select specific platforms if you have specific redeem requirements.
                                        </p>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        {Object.values(Platform).map((platform) => (
                                            ...
                                            >
                                                <div className="flex items-center justify-between">
                                                    <span className={`font-bold text-sm tracking-wide ${platforms.includes(platform) ? 'text-white' : 'text-bl-gray-light'}`}>
                                                        {platform}
                                                    </span>
                                                    <div className={`
                                                        w-4 h-4 border flex items-center justify-center transition-colors
                                                    ...
                                                    `}>
                                                        {platforms.includes(platform) && <Check className="w-3 h-3" />}
                                                    </div>
                                                </div>
                                            </button>
                                        ))}
                            ...
                            {step === 2 && (
                                <div className="space-y-6 max-w-xl mx-auto">
                                    ...
                                    <div className="flex items-start justify-between p-4 bg-black/40 border border-bl-gray-dark">
                                        <div>
                                            <h3 className="text-base font-bold text-white mb-1">Auto-Redeem Codes</h3>
                                            <p className="text-xs text-bl-gray-light">
                                                Automatically check for and redeem new codes in the background.
                                            </p>
                                        </div>
                                        <button
                                    ...
                                    {autoRedeem && (
                                        <div className="space-y-4 animate-in slide-in-from-top-4 fade-in">
                                            ...
                                            <div className="space-y-2">
                                                <label className="block text-xs font-bold uppercase text-bl-gray-light tracking-wider">
                                                    Check Frequency
                                                </label>
                                                <select
                                                    value={checkInterval}
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <label onClick={() => setNotify(!notify)} className="cursor-pointer text-sm text-white select-none">
                        ...
                        <div className="p-6 border-t border-bl-gray-dark/50 flex justify-between bg-black/20">
                            <button
                                ...
                                className={`
                                    ...
                                    ${step === 0 ? 'opacity-0 pointer-events-none' : 'text-bl-gray-light hover:text-white'}
                                `}
                            >
